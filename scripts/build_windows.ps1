...
.venv\Scripts\python.exe -m PyInstaller --noconfirm --clean --windowed --onedir --name $name --icon "assets/图标.ico" --paths src --add-data "src/opencover/workers/rvc_training_runtime.py;workers" --add-data "assets;assets" --add-data "config;config" --add-data "src/opencover/workers/vevo2_runtime.py;workers" --add-data "src/opencover/workers/diffsinger_legacy_runtime.py;workers" --add-data "src/opencover/workers/espnet_visinger2_runtime.py;workers" --add-data "src/opencover/workers/alignment_runtime.py;workers" --add-data "src/opencover/workers/score_refinement_runtime.py;workers" --add-data "src/opencover/workers/rvc_batch_runtime.py;workers" app.py
$release = Join-Path 'dist' $name
.venv\Scripts\python.exe -m PyInstaller --noconfirm --clean --console --onefile --name OpenCoverStudioWorker --paths src --add-data "src/opencover/workers/rvc_training_runtime.py;workers" --add-data "src/opencover/workers/vevo2_runtime.py;workers" --add-data "src/opencover/workers/diffsinger_legacy_runtime.py;workers" --add-data "src/opencover/workers/espnet_visinger2_runtime.py;workers" --add-data "src/opencover/workers/alignment_runtime.py;workers" --add-data "src/opencover/workers/score_refinement_runtime.py;workers" --add-data "src/opencover/workers/rvc_batch_runtime.py;workers" worker_entry.py
