...
14. 测试：37 项 pytest 全部通过；完整数值与文件哈希见 `docs/TEST_REPORT.md`。
15. 发行：GUI 为 windowed EXE；独立 worker 用 `CREATE_NO_WINDOW` 隐藏启动并保留 UTF-8 进度。Modern/Legacy 公开包均为 641.8 MiB/467 文件，敏感资源泄漏数为 0；本机 Modern 完整私有包加入独立对齐环境后约 26.58 GiB，打包后 Vevo2 改词、低显存 DiffSinger 回退、Whisper 强制对齐与取消均通过。
...
20. 未完成：每引擎 3～5 个可再分发初始音色、完全可移植的所有重型后端公开扩展包和实体旧显卡 Legacy 验收。界面不会把它们伪装为可用。
