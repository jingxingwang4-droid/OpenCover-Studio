...
14. 测试：34 项 pytest 全部通过；完整数值与文件哈希见 `docs/TEST_REPORT.md`。
15. 发行：GUI 为 windowed EXE；独立 worker 用 `CREATE_NO_WINDOW` 隐藏启动并保留 UTF-8 进度。Modern/Legacy 公开包均为 641.8 MiB/466 文件且敏感资源泄漏数为 0。本机 Modern 完整私有包约 23.65 GiB，打包后 Vevo2 改词、低显存 DiffSinger 回退与取消均通过。
