...
class AudioPlayer(QFrame):
    def __init__(self, parent: QWidget | None = None):
        ...
        self.volume.valueChanged.connect(lambda value: self.output.setVolume(value / 100.0))
...
class VoiceCard(QFrame):
    ...
    def __init__(self, model, directory: Path, parent: QWidget | None = None):  # type: ignore[no-untyped-def]
        ...
        quality = {"verified": "已验证", "experimental": "实验性", "rejected": "不推荐"}.get(model.quality_status, "实验性")
        tag = QLabel(f"{model.engine.upper()} · {'内置' if model.bundled else '用户导入'} · {quality}")
        ...
        choose.setEnabled(model.selectable and model.quality_status != "rejected")
        if not choose.isEnabled():
            choose.setToolTip("该音色已在真实歌曲验证中因质量不合格被停用")
