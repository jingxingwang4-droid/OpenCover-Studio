...
OpenCover Studio 是面向 Windows 普通用户的本地歌曲翻唱桌面应用。当前仓库是 **v0.1.0 阶段性可运行版本**：桌面 GUI、模型导入、任务隔离、SQLite、资源下载安全边界、音频标准化/混音模块和后端适配器已经实现；大型推理后端与权重尚未完成本机安装和真实推理验证，因此软件会明确显示“未安装”，不会输出伪造音频。
...
4. 拖入歌曲，先选 RVC/DDSP，再选对应音色，点击“开始翻唱”。

当前尚未提供合法可再分发的标准歌唱干声，所以“自动生成试听”未宣称可用。资源真实状态详见 `config/resource_manifest.yaml` 与 `docs/RESOURCE_RESEARCH.md`。
...
本机阶段构建位于 `dist/OpenCoverStudio-NVIDIA-Modern` 与 `dist/OpenCoverStudio-NVIDIA-Legacy`，各 575.3 MB。`dist/` 按要求不进入 Git；详细证据见 `docs/TEST_REPORT.md` 和 `docs/FINAL_REPORT.md`。
